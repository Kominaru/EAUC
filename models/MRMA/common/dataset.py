...
class DatasetManager:
    ...
    def _init_data(self):
        if self.kind == self.KIND_MOVIELENS_100K:
            ...
            self.__init_data('/ratings.csv', ',', header=True)
        else:
            raise NotImplementedError()
    ...
    def _split_data(self):
        ...
        np.save('data/{}/train-data.npy'.format(self.kind), data[0:n_train, :])
        np.save('data/{}/valid-data.npy'.format(self.kind), data[n_train:, :])
