...
class MovieLens(object):
    ...
    def _generate_pair_value(self, rating_info):
        rating_pairs = (
            np.array(
                [self.global_user_id_map[ele] for ele in rating_info["user_id"]],
                dtype=np.int64,
            ),
            np.array(
                [self.global_movie_id_map[ele] for ele in rating_info["movie_id"]],
                dtype=np.int64,
            ),
        )
    ...
    def _generate_enc_graph(self, rating_pairs, rating_values, add_support=False):
        ...
        # sanity check
        assert len(rating_pairs[0]) == sum([graph.num_edges(et) for et in graph.etypes]) // 2
