...
def evaluate(args, net, dataset, segment="valid"):
    ...
    if segment == "test":
        dataset.test_rating_info["pred"] = real_pred_ratings.cpu().numpy()
...
def train(args):
    ...
    for iter_idx in range(1, args.train_max_iter):
        ...
        if iter_idx == 1:
            ...
            print(
                torch_net_info(
                    net,
                    save_path=os.path.join(args.save_dir, "net%d.txt" % args.save_id),
                )
            )
        ...
        if iter_idx % args.train_valid_interval == 0:
            ...
            if valid_rmse < best_valid_rmse:
                ...
                dataset.train_rating_info["pred"] = real_pred_ratings.detach().cpu().numpy()
                best_test_rmse = test_rmse
                test_loss_logger.log(iter=iter_idx, rmse=test_rmse)
                logging_str += ", Test RMSE={:.4f}".format(test_rmse)
            else:
                no_better_valid += 1
                if no_better_valid > args.train_early_stopping_patience and learning_rate <= args.train_min_lr:
    ...
    # Export test and train rating dfs to csv
    os.makedirs("outputs/GC_MC", exist_ok=True)
