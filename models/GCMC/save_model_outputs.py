...
def save_model_outputs(train_df, test_df, model_name, dataset_name, model_params):
    ...
    if not os.path.exists(f"{OUTPUTS_DIR}/{dataset_name}/{model_name}"):
        os.makedirs(f"{OUTPUTS_DIR}/{dataset_name}/{model_name}")

    # Get list of files in outputs/{model_name} folder
    files = os.listdir(f"{OUTPUTS_DIR}/{dataset_name}/{model_name}")
    ...
    train_df.to_csv(f"{OUTPUTS_DIR}/{dataset_name}/{model_name}/train_outputs_{i}.csv", index=False)
    test_df.to_csv(f"{OUTPUTS_DIR}/{dataset_name}/{model_name}/test_outputs_{i}.csv", index=False)

    print(
        f"Saved train and test outputs to {OUTPUTS_DIR}/{dataset_name}/{model_name}/train_outputs_{i}.csv and {OUTPUTS_DIR}/{dataset_name}/{model_name}/test_outputs_{i}.csv"
    )

    # Write model_params to params.txt file
    with open(f"{OUTPUTS_DIR}/{dataset_name}/{model_name}/params.txt", "a") as f:
