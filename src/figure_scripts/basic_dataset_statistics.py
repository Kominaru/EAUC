...
def plot_basic_dataset_statistics_figures(all_samples : pd.DataFrame, train_samples : pd.DataFrame, MODEL_NAME : pd.DataFrame) -> None:
    '''
    ...
    train_samples.groupby('movie_id')['rating'].mean().plot.hist(bins=25, title='Histogram of Average Movie Ratings',  logy=True)
    ...
    train_samples.groupby('user_id')['rating'].mean().plot.hist(bins=25, title='Histogram of Average User Ratings',  logy=True)
    plt.xlabel('Average Rating')
    plt.ylabel('# of users')
