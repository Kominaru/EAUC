...
def plot_2heatmaps_grid_by_unique_ratings(
    train_samples: pd.DataFrame, test_samples: pd.DataFrame, MODEL_NAME: str, bin_interval: float = 0.5
) -> None:
    ...
    # Calculate the average rating per user and item in the train set
    ...
    test_samples["item_avg_rating"] = test_samples["item_id"].map(item_avg_ratings).reset_index(drop=True)

    test_samples["user_bin"] = pd.cut(test_samples["user_avg_rating"], bins=bins, include_lowest=True)
    test_samples["item_bin"] = pd.cut(test_samples["item_avg_rating"], bins=bins, include_lowest=True)

    train_samples["user_avg_rating"] = train_samples["user_id"].map(user_avg_ratings).reset_index(drop=True)
    train_samples["item_avg_rating"] = train_samples["item_id"].map(item_avg_ratings).reset_index(drop=True)

    train_samples["user_bin"] = pd.cut(train_samples["user_avg_rating"], bins=bins, include_lowest=True)
    train_samples["item_bin"] = pd.cut(train_samples["item_avg_rating"], bins=bins, include_lowest=True)
    ...
    for i, rating in enumerate(ratings):
        ...
        rmse_table = (
            test_samples[test_samples["rating"] == rating]
            .groupby(["user_bin", "item_bin"])["error"]
            .apply(lambda x: np.sqrt(np.mean(x**2)))
            .reset_index(name="rmse")
        )
        pivot_table = rmse_table.pivot(index="user_bin", columns="item_bin", values="rmse")
        sns.heatmap(
            pivot_table.iloc[::-1],
            cmap=cmap,
            annot=False,
        ...
        frequency_table = (
            test_samples[test_samples["rating"] == rating]
            .groupby(["user_bin", "item_bin"])
            .size()
            .reset_index(name="frequency")
        )
        frequency_table["log_frequency"] = frequency_table["frequency"].apply(
            lambda x: np.NaN if x == 0 else round(np.log10(x))
        )
        ...
        sns.heatmap(
            ...
            cmap="YlGnBu",
            annot=False,
            cbar_kws={"label": "Logarithmic Frequency"},
            fmt="g",
            ax=axs[i, 1],
            vmin=0,
        )
        axs[i, 1].set_xlabel("Item's Average Rating")
        ...
        frequency_table = (
            train_samples[train_samples["rating"] == rating]
            .groupby(["user_bin", "item_bin"])
            .size()
            .reset_index(name="frequency")
        )
        ...
        axs[i, 2].set_xlabel("Item's Average Rating")
