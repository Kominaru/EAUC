...
def plot_ratings_vs_preds_2dheatmaps_grid(train_samples: pd.DataFrame, test_samples: pd.DataFrame, model_name:str, preds_bin_interval: int = 0.5, avgs_bin_interval: int = 0.5) -> None:
    ...
    frequency_table = test_samples.groupby(['user_bin', 'movie_bin', 'rating', 'pred_bin']).size().reset_index(name='frequency')
    ...
    for i, user_bin in enumerate(user_bins):
        for j, movie_bin in enumerate(user_bins):
            ...
            # Plot an x=y line
            axs[i, j].plot([0, test_samples['rating'].nunique()], [0, test_samples['pred_bin'].nunique()], color='black', linewidth=1)

    plt.tight_layout()
    plt.savefig('figures/' + model_name + '/ratings_vs_preds_2d_heatmaps_grid.pdf')

    # Second figure: for each user and movie, plot a line plot of the rating vs average prediction and standard deviation of predictions
    fig, axs = plt.subplots(len(user_bins), len(user_bins), figsize=(5*(len(user_bins)), 5*(len(user_bins))))

    for i, user_bin in enumerate(user_bins):
        for j, movie_bin in enumerate(user_bins):

            # Select the samples inside the current user_bin and movie_bin
            df = test_samples[(test_samples['user_bin'] == user_bin) & (test_samples['movie_bin'] == movie_bin)]
            ...
            axs[i, j].fill_between(df['rating'], df['mean'] - df['std'], df['mean'] + df['std'], alpha=0.2, color='blue')

            # Plot the x=y line
            axs[i, j].plot([1, test_samples['rating'].max()], [1, test_samples['pred'].max()], color='black', linewidth=1)
            ...
            # Plot a red dot at the average of the average user and movie ratings
            axs[i, j].plot([user_bin.mid*.5 + movie_bin.mid*.5], [user_bin.mid*.5 + movie_bin.mid*.5], color='red', marker='o', markersize=5)
