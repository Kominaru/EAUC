...
def all_ratings_2dheatmap(
    train_samples: pd.DataFrame, test_samples: pd.DataFrame, MODEL_NAME: str, bin_interval: int = 0.5
) -> None:
    ...
    frequency_table = train_samples.groupby(["user_bin", "item_bin"]).size().reset_index(name="frequency")
    frequency_table["log_frequency"] = frequency_table["frequency"].apply(
        lambda x: np.NaN if x == 0 else round(np.log10(x))
    )

    # Compute the RMSE in each (user average rating, item average rating) bin
    test_samples["error"] = test_samples["rating"] - test_samples["pred"]
    rmse_table = (
        test_samples.groupby(["user_bin", "item_bin"])["error"]
    ...
    pivot_table_freq = frequency_table.pivot(index="user_bin", columns="item_bin", values="log_frequency")
    pivot_table_rmse = rmse_table.pivot(index="user_bin", columns="item_bin", values="rmse")
