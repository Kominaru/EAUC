...
def plot_avg_bias_error(train_samples_dict: dict, test_samples_dict: dict = None,  bins : int = 100) -> None:
    '''
    Plots the error of the models depending on the distance from the real rating to the average of the user and movie average ratings
    ...
    for model_name in train_samples_dict:
        ...
        # Map each sample to its user and movie average rating
        test_samples['user_avg_rating'] = test_samples['user_id'].map(train_samples.groupby('user_id')['rating'].mean())
        test_samples['movie_avg_rating'] = test_samples['movie_id'].map(train_samples.groupby('movie_id')['rating'].mean())

        # Compute the distance from the real rating to the average of the user and movie average ratings
        test_samples['dist_to_avg_rating'] = test_samples['rating'] - (test_samples['user_avg_rating'] + test_samples['movie_avg_rating']) / 2
    ...
    plt.plot([min_dist, min_dist, max_dist, max_dist, min_dist], [0, (max_rating-min_rating), (max_rating-min_rating), 0, 0], label='Maximum area')
...
if __name__ == '__main__':
    ...
    for model in models:
        ...
        test_avg_bias_auc = compute_avg_bias_auc(model_predictions[model]['train'], model_predictions[model]['test'], method=auc_method, bins=20)
